...
from transformers import AdamW, RobertaModel, RobertaConfig, RobertaTokenizer
import torch
...
# create a logger
pretrained_path = config['trans_modelpath']
log_dir_path = './logs/transpoly/'
if not os.path.exists(log_dir_path):
    os.makedirs(log_dir_path)
log_filename = os.path.join(log_dir_path, f"transpoly_{os.path.splitext(os.path.basename(config['downstream_dataset']))[0]}_{config['activation']}_lr_{config['lr']}_{config['batch_size']}_nepochs_{config['n_epochs']}_dropout_{config['dropout_ratio']}_{config['k_fold']}_hidden_{config['hidden_size']}_decay_{config['weight_decay']}.log")
...
def main(config):
    ...
    for fold_num, dataloader in enumerate(train_dataloader):
        ...
        PretrainedModel = RobertaModel.from_pretrained(config['trans_modelpath'])
        ...
        model = polycl.polycl_pred(PretrainedModel, drop_ratio = config['dropout_ratio'], hidden_size = config['hidden_size'], activation_func = activation)
        loss_fn = nn.MSELoss()
        model.to(device)
